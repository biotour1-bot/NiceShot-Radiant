...
const MS = ({ icon, filled = false, className = '', size = 24, color }) => (
  <span
...
// --- Legacy SVG Icons (kept for backward compat) ---
const Icons = {
  Home: ({ active }) => <MS icon="home" filled={active} color={active ? 'var(--accent-blue-light)' : undefined} />,
  Action: ({ active }) => <MS icon="groups" filled={active} color={active ? 'var(--accent-blue-light)' : undefined} />,
  Rank: ({ active }) => <MS icon="leaderboard" filled={active} color={active ? 'var(--accent-gold)' : undefined} />,
  Users: ({ active }) => <MS icon="manage_accounts" filled={active} color={active ? 'var(--accent-blue-light)' : undefined} />,
  Settings: ({ active }) => <MS icon="calendar_month" filled={active} color={active ? 'var(--accent-blue-light)' : undefined} />,
  Notice: ({ active }) => <MS icon="campaign" filled={active} color={active ? 'var(--accent-gold)' : undefined} />,
  AdminBoard: ({ active }) => <MS icon="edit_note" filled={active} color={active ? 'var(--accent-blue-light)' : undefined} />,
};
...
const AdminDashboard = ({ eventData, viewMode, notices, setSubView }) => (
  <div className="animate-fade-in section-spacing">
     ...
     {/* Featured Tournament Card - Stitch Style */}
     <div className="featured-card mb-6">
       <div style={{ position: 'absolute', top: 0, right: 0, padding: 16, opacity: 0.08 }}>
         <MS icon="sports_golf" size={72} />
       </div>
       <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
         <MS icon="star" filled size={16} color="var(--accent-gold)" />
         <span style={{ color: 'var(--accent-gold)', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Upcoming Tournament</span>
       </div>
       <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.3rem', fontWeight: 700, color: 'white', marginBottom: 12 }}>
         {eventData?.name || '준비 중...'}
       </h2>
       <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 16 }}>
         <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--on-surface-variant)', fontSize: '0.8rem' }}>
           <MS icon="calendar_month" size={16} />
           <span>{eventData?.date || '일정 미정'}</span>
         </div>
         <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--on-surface-variant)', fontSize: '0.8rem' }}>
       ...
       <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.65rem', color: 'var(--on-surface-variant)', marginBottom: 6, fontWeight: 700 }}>
         <span>팀 등록 현황</span>
         <span style={{ color: 'var(--accent-blue-light)' }}>{eventData?.teamCount || 0} 팀</span>
       </div>
       <div className="progress-bar">
         <div className="progress-fill" style={{ width: `${Math.min(100, ((eventData?.teamCount || 0) / 6) * 100)}%` }} />
       </div>
     </div>

     <div className="glass-panel p-5 mb-4" style={{ background: 'linear-gradient(135deg, rgba(26,74,154,0.1), rgba(0,0,0,0))' }}>
        {viewMode === 'ADMIN' ? (
          <>
            {/* Stitch Quick Actions Grid */}
            <div className="grid grid-cols-2 gap-3 mb-3">
               <button className="quick-action-card" onClick={() => setSubView('MEMBER_MGMT')}>
                 <div className="quick-action-icon">
                   <MS icon="manage_accounts" size={24} color="var(--accent-blue-light)" filled />
                 </div>
                 <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--on-surface)', fontFamily: 'var(--font-heading)', textAlign: 'center' }}>회원 관리</span>
               </button>
               <button className="quick-action-card" onClick={() => setSubView('EVENT_SETUP')}>
                 <div className="quick-action-icon">
                   <MS icon="calendar_month" size={24} color="var(--accent-blue-light)" filled />
                 </div>
                 <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--on-surface)', fontFamily: 'var(--font-heading)', textAlign: 'center' }}>대회 설정</span>
               </button>
               <button className="quick-action-card" onClick={() => setSubView('NOTICES')}>
                 <div className="quick-action-icon">
                   <MS icon="campaign" size={24} color="var(--accent-gold)" filled />
                 </div>
                 <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--on-surface)', fontFamily: 'var(--font-heading)', textAlign: 'center' }}>공지사항</span>
               </button>
               <button className="quick-action-card" onClick={() => setSubView('SCORE_MGMT')}>
                 <div className="quick-action-icon" style={{ background: 'rgba(26,74,154,0.25)', border: '1px solid rgba(26,74,154,0.3)' }}>
                   <MS icon="scoreboard" size={24} color="var(--accent-blue-light)" filled />
                 </div>
                 <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--on-surface)', fontFamily: 'var(--font-heading)', textAlign: 'center' }}>스코어 관리</span>
               </button>
            </div>
            <button
              className="glass-panel w-full py-3"
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12 }}
              onClick={() => setSubView('ADMIN_BOARD')}
            >
              <MS icon="edit_note" size={18} color="var(--on-surface-variant)" />
              <span style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--on-surface-variant)', fontFamily: 'var(--font-heading)' }}>운영게시판</span>
            </button>
          </>
        ) : (
              <div className="space-y-4 text-left">
                 <div className="flex items-center gap-2 mb-2">
                    <MS icon="campaign" size={18} color="var(--accent-gold)" filled />
     ...
     <div className="grid grid-cols-2 gap-4">
        <div className="glass-panel p-5 flex flex-col items-center justify-center border-white/5">
           <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
             <MS icon="groups" size={14} color="var(--on-surface-variant)" />
             <span className="text-[10px] text-dim font-bold uppercase">총 팀 수</span>
           </div>
        ...
        <div className="glass-panel p-5 flex flex-col items-center justify-center border-accent/20">
           <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
             <MS icon="schedule" size={14} color="var(--accent-gold)" />
             <span className="text-[10px] text-accent font-bold uppercase">첫조 티오프</span>
           </div>
...
function App() {
  ...
  return (
    <div className="min-h-screen container" style={{ paddingTop: 0 }}>
      {/* Stitch-style sticky header */}
      <header className="app-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <MS icon="golf_course" size={22} color="var(--accent-blue-light)" filled />
          <span style={{
            ...
            textShadow: '0 0 12px rgba(175,198,255,0.5)',
          }}>NiceShot</span>
          {(activeTab !== 'HOME' || subView !== 'MAIN') && (
            <span style={{ fontSize: '0.6rem', color: 'var(--on-surface-variant)', fontWeight: 600, marginLeft: 4, opacity: 0.6 }}>{viewMode}</span>
          )}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button style={{ background: 'transparent', padding: 4 }}>
            <MS icon="notifications" size={22} color="var(--on-surface-variant)" />
          </button>
        </div>
      </header>
      ...
      <div style={{ padding: '16px 0' }}>
       ...
       {/* Stitch Pill Navigation Bar */}
       <div className="nav-container">
         <nav className="nav-bar">
           {[
             { id: 'HOME',   icon: 'home',       label: '홈',    gold: false },
             { id: 'ACTION', icon: 'groups',      label: '조편성', gold: false },
             { id: 'RANK',   icon: 'leaderboard', label: '순위',  gold: true  },
           ].map(t => {
             const isActive = activeTab === t.id;
             const activeColor = t.gold ? 'var(--accent-gold)' : 'var(--accent-blue-light)';
             const glowColor = t.gold ? 'rgba(251,175,63,0.8)' : 'rgba(175,198,255,0.8)';
             return (
               <button
                 key={t.id}
                 className={`nav-item${isActive ? ' active' : ''}`}
                 onClick={() => { setActiveTab(t.id); setSubView('MAIN'); }}
                 style={isActive ? { color: activeColor, filter: `drop-shadow(0 0 8px ${glowColor})` } : {}}
               >
                 <MS icon={t.icon} filled={isActive} size={22}
                   color={isActive ? activeColor : 'rgba(195,198,211,0.45)'}
                 />
